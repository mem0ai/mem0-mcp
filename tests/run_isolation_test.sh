#!/bin/bash
# Run project isolation test for mem0-mcp
...
if [ "$1" = "-v" ] || [ "$1" = "--verbose" ]; then
    echo "Running project isolation test with verbose output..."
    uv run -m tests.test_project_isolation --verbose
else
    echo "Running project isolation test..."
    uv run -m tests.test_project_isolation
fi 